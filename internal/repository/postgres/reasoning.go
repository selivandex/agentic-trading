...
func (r *ReasoningRepository) Create(ctx context.Context, entry *reasoning.LogEntry) error {
	query := `
		INSERT INTO agent_reasoning_logs (
			id, user_id, agent_id, agent_type, session_id, symbol, trading_pair_id,
	...
	_, err := r.db.ExecContext(ctx, query,
		entry.ID, entry.UserID, entry.AgentID, entry.AgentType, entry.SessionID, entry.Symbol, entry.TradingPairID,
