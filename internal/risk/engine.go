...
func (e *RiskEngine) RecordTrade(ctx context.Context, userID uuid.UUID, pnl decimal.Decimal) error {
	...
	// Calculate PnL percentage (assuming we have initial balance)
	// For now, we'll use absolute PnL and let the user configure thresholds
	state.DailyPnLPercent = state.DailyPnL // TODO: calculate actual percentage
