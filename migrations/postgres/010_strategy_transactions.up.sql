...
CREATE TYPE strategy_transaction_type AS ENUM (
    'deposit',           -- Initial deposit or additional funding
    'withdrawal',        -- Withdraw funds from strategy
    'position_open',     -- Cash allocated to open position
    'position_close',    -- Position closed, cash returned (with PnL)
    'fee',              -- Trading fee deduction
    'funding_rate',     -- Funding rate payment (perpetual futures)
...
CREATE INDEX idx_strategy_transactions_position_id ON strategy_transactions(position_id) WHERE position_id IS NOT NULL;
