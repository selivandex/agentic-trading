...
PARTITION BY
    toYYYYMM (timestamp)
ORDER BY
    (symbol, timestamp) TTL timestamp + INTERVAL 1 YEAR;

-- Materialized view for hourly feature aggregation
CREATE MATERIALIZED VIEW IF NOT EXISTS regime_features_hourly_mv ENGINE = AggregatingMergeTree ()
PARTITION BY
    toYYYYMM (hour)
ORDER BY
    (symbol, hour) AS
